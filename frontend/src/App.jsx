...
function App() {
  ...
  const fetchPortfolio = async () => {
    try {
      ...
      const mappedData = data.map(dto => ({
        ...
        qty: dto.amount,
        avgCost: dto.avgBuyPrice,
        currentPrice: dto.currentValue / dto.amount, // Derived or estimated
        totalValue: dto.totalValue,
        profitLoss: dto.profitLoss,
        icon: dto.symbol ? dto.symbol[0] : '?',
  ...
  const handleAddHolding = async (e) => {
    ...
    const payload = {
      ...
      name: form.name?.value || form.symbol.value.toUpperCase(),
      quantity: parseFloat(form.qty.value),
      avgBuyPrice: parseFloat(form.avgCost.value),
      source: "Manual Entry"
    };
  ...
  const handleEditHolding = async (e) => {
    ...
    const payload = {
      ...
      avgBuyPrice: parseFloat(e.target.avgCost.value),
      source: selectedAsset.source || "Manual Entry"
    };
