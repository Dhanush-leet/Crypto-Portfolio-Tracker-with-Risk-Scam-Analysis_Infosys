...
public class AssetService {
    ...
    public AssetResponseDTO addAsset(Long userId, AssetRequestDTO request) {
        ...
        java.util.Optional<Asset> existing = assetRepository.findByUserIdAndSymbol(userId, request.getSymbol());
        ...
        if (existing.isPresent()) {
            ...
            BigDecimal newPrice = request.getAvgBuyPrice();

            BigDecimal currentTotalQty = asset.getAmount();
            BigDecimal currentAvgPrice = asset.getAvgBuyPrice();

            BigDecimal currentCost = currentTotalQty.multiply(currentAvgPrice);
            BigDecimal newCost = newQty.multiply(newPrice);

            BigDecimal totalQty = currentTotalQty.add(newQty);

            if (totalQty.compareTo(BigDecimal.ZERO) > 0) {
                BigDecimal avgPrice = currentCost.add(newCost).divide(totalQty, RoundingMode.HALF_UP);
        ...
        // Record Initial BUY Trade (or subsequent BUY)
        recordTrade(user, asset.getSymbol(), "BUY", request.getQuantity().doubleValue(),
